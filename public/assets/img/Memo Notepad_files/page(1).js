...
(function() {
    ...
    */
    Page._createOrUpdateListingForNote = function(args, cbk) {
        if (!args.id) { return cbk([0, "Expected note id for listing"]); }
        ...
        async.auto({
            ...
            createOrUpdateListing: ["getNote", function(go_on, res) {
                var id = args.id;
                var note = res.getNote;

                if (!note) { return go_on(); }
                ...
                if (!isSelected && isHidden) { return go_on(); }

                if (!!existingNodes.length) { existing = $(existingNodes[0]); }
                ...
                listing
                    .addClass("stack")
                    .data("id", id)
                    .data("lm", note.text_last_modified)
                    .removeClass("highlight")
                    .toggleClass("empty", isHidden)
                    .toggleClass("notice", isNotice)
                    .toggleClass("selected", isSelected);

                if (!!existing) { return go_on(); }

                return go_on(null, listing);
            }],

            insertListing: ["createOrUpdateListing", function(go_on, res) {
                var note = res.getNote;

                if (!res.createOrUpdateListing || !note) { return go_on(); }
                ...
                Page._insertListingInSortedList({
                    listing: listing,
                    note: note
                });

                if (!!args.skip_animation) { listing.addClass("added"); }

                setTimeout(function() { listing.addClass("added"); }, 20);
    ...
    /** Insert a listing into a sorted existing list

        {
            [adjust_position]: <Bool> = false
            listing: <DOM Note Object>
            note: <Note Object>
        }
    */
    Page._insertListingInSortedList = function(args) {
        ...
        $("#stacks .stack").each(function() {
            ...
            if (args.note.text_last_modified < $(this).data("lm")) { return; }

            olderNode = $(this);
        });

        if (!olderNode) { return $("#stacks").append(args.listing); }

        if (!!args.adjust_position &&
            Math.abs(args.listing.index() - olderNode.index()) < 2)
        {
            return;
        }
        ...
        if (Page.getSelectedNoteId({}) === args.note.id) { return; }

        $(args.listing).removeClass("added");

        setTimeout(function() { args.listing.addClass("added"); }, 20);
    };
    ...
    */
    Page._logoutRestart = function(args, cbk) {
        async.auto({
            ...
            clearDatabase: ["resetSync", function(go_on, res) {
                Data.logout({}, go_on);
            }],

            restartDatabase: ["clearDatabase", function(go_on, res) {
                Data.start({}, go_on);
            }],

            restartPage: ["restartDatabase", function(go_on, res) {
    ...
    */
    ...
    /** Update the note last time edited indicator

        {
            text: <String>
            text_last_modified: <ISO8601 Date String>
        }
    */
    Page._noteTime = function(args, cbk) {
        ...
        if (args.text === "") { display.text(""); return cbk(); }
    ...
    /** Purchase a premium subscription with a card token

        {
            card_token: <String>
            plan: <String>
            purchase_button: <DOM Element Object>
        }
    */
    Page._purchasePremiumSubscriptionWithCardToken = function(args) {
        async.auto({
            ...
            indicateLoading: ["validateArguments", function(go_on, res) {
                args.purchase_button.button("loading");

                go_on();
            }],

            initiatePurchase: ["indicateLoading", function(go_on, res) {
                Sync.purchaseStripeSubscription({
            ...
            refreshAccount: ["initiatePurchase", function(go_on, res) {
    ...
    /** Scroll an element into view, with scroll animation, if it's hidden
    ...
    */
    Page._scrollIntoView = function(args) {
        if (!args.element || !args.element.scrollIntoView) { return; }
    ...
    */
    ...
    /** Select the next note in the note listing
    ...
    */
    Page._selectNextListing = function(args) {
        ...
        if (!!next.length) { return next.click(); }

        var prev = listing.prev(".stack:not(.empty)");

        if (!!prev.length) { return prev.click(); }

        return;
    };

    /** Display search results in note listings

        {
            [notes]: Array<Note Object>
            [show_all]: <Bool> = false
            [show_single]: <Id String> = undefined
        }
    ...
    */
    ...
    /** Toggle purchase background theme button
    ...
    */
    ...
    /** Track the state through purchase flow
    ...
    */
    Page._trackPurchaseState = function(args) {
        Client.track({
            path: ["purchase", args.product_key, args.state].join("/")
        });
    };

    /** Eliminate the existing note and move the current note selection
    ...
    */
    Page._trashNote = function(args, cbk) {
        async.auto({
            ...
            getNote: ["selectedNoteId", function(go_on, res) {
                if (!res.selectedNoteId) { return go_on([0, "Expected id"]); }

                Data.getNote({id: res.selectedNoteId}, go_on);
            }],

            getOlderNote: ["getNote", function(go_on, res) {
                if (!res.getNote) { return go_on([0, "Expected note"]); }

                Data.getNextNote({
                    before: res.getNote.text_last_modified,
                    exclude_id: Page.getSelectedNoteId({})
                },
                go_on);
            }],

            getNewerNote: ["getNote", "getOlderNote", function(go_on, res) {
                if (res.getOlderNote !== undefined) { return go_on(); }

                Data.getNextNote({
                    after: res.getNote.text_last_modified,
                    exclude_id: Page.getSelectedNoteId({})
                },
                go_on);
            }],
            ...
            // Animate to the newly selected note
            animateSelection: ["noteToSwitchTo", function(go_on, res) {
                if (!!res.createNewNote) { return go_on(); }

                Page._noteListingElementsById({id: res.selectedNoteId})
                    .addClass("removed");

                Page._noteListingElementsById({id: res.noteToSwitchTo.id})
                    .addClass("selected");

                setTimeout(go_on, 200);
            }],

            reuseExisting: ["getNote", "noteToSwitchTo", function(go_on, res) {
                var reusedExisting = res.noteToSwitchTo.id === res.getNote.id;

                go_on(null, reusedExisting);
            }],

            eliminateRemoved: ["animateSelection", function(go_on, res) {
                if (!!res.reuseExisting) { return go_on(); }
    ...
    /** Determine if there was typing recently
    ...
    */
    Page.addAnonNotice = function(args, cbk) {
        ...
        if (!args.id || !args.content) { return cbk(); }

        async.auto({
            addNotice: function(go_on) {
                Data.addNote({
                    destination: args.destination,
                    destination_icon: args.destination_icon,
                    id: Data.uuid({}),
                    text: args.content
                },
                go_on);
            ...
            updateStacks: ["addNotice", function(go_on, res) {
                if (!!res.getExistingNote) { return go_on(); }
    ...
    */
    Page.addNewNote = function(args, cbk) {
        async.auto({
            ...
            addNote: ["save", function(go_on, res) {
            ...
            showNote: ["addNote", function(go_on, res) {
                Client.track({path: "stacks/new"});

                Page.selectNote({id: res.addNote.id}, go_on);
            }],

            showNoteListing: ["showNote", function(go_on, res) {
                Page._createOrUpdateListingForNote({
                    id: res.addNote.id
                },
                go_on);
    ...
    /** Add a note to explain remote sync
    ...
    */
    Page.addRemoteSyncInfoNote = function(args, cbk) {
        if (!args.service) { return cbk([0, "Expected remote sync service"]); }
        ...
        async.auto({
            ...
            addNote: ["dropbox", "google", function(go_on, res) {
                var text = res[args.service];

                if (!text) { return go_on([0, "Expected known service"]); }
    ...
    Page.adjustFontSize = function(args, cbk) {
        async.auto({
            getAdjustedSize: function(go_on) {
                ...
                if (!_(_FONT_SIZES).contains(adjustedSize)) { return go_on(); }

                go_on(null, adjustedSize);
            },

            adjustFontSize: ["getAdjustedSize", function(go_on, res) {
                if (res.getAdjustedSize === undefined) { return go_on(); }
            ...
            saveFontSize: ["getAdjustedSize", function(go_on, res) {
                if (res.getAdjustedSize === undefined) { return go_on(); }

                Client.setSetting({
                    setting: "font_size",
                    value: res.getAdjustedSize
                },
                go_on);
            }]
        },
    ...
    */
    Page.adjustFontStyle = function(args, cbk) {
        if (!args.style) { return cbk([0, "Expected style"]); }
    ...
    /** Adjust elements to the window size
    ...
    */
    Page.adjustSizeToWindowHeight = function(args) {
        ...
        $("#stacks").css({
            height: windowHeight - $("#stacks").offset().top - 50
        });
    };
    ...
    /** Advance registration step
        ...
        {
            register_button: <DOM Object>
        }
    */
    Page.advanceRegistration = function(args, cbk) {
        if (!args.register_button) { return cbk([0, "Expected button"]); }
        ...
        Client.clearStorage({}, function(err) {
            ...
            var completeRegistrationTitle = $(args.register_button)
                .data("complete_registration_title");
    ...
    /** Trigger authentication with the auth service
    ...
    */
    Page.authenticate = function(args) {
        ...
        if (!email) { return Page._authFail({code: "missing_email"}); }

        if (!password) { return Page._authFail({code: "missing_password"}); }
        ...
        Sync.start({
            register: $("#account").is(".registering")
        },
        function(err) {
            if (!!err) { return Page.showError({err: err}); }
        });
    ...
    /** Switch page to authorized mode
    ...
    */
    Page.authorized = function(args, cbk) {
        async.auto({
            ...
            showAuthenticated: ["hasAuth", "getEmail", function(go_on, res) {
                if (!res.hasAuth && !args.force) { return go_on(); }

                if (!res.getEmail) { return go_on(); }
    ...
    Page.changeBackground = function(args, cbk) {
        async.auto({
            ...
            removePremiumButtons: function(go_on) {
                $("#account")
                    .removeClass("google_drive_sync_info dropbox_sync_info");

                go_on();
            },

            setBackground: ["backgroundType", function(go_on, res) {
                $("body")
                    .removeClass("dark standard")
                    .addClass(res.backgroundType);

                return go_on();
            }],

            saveBackground: ["setBackground", function(go_on, res) {
                Client.track({path: "account/background/"+res.backgroundType});

                Client.setSetting({
                    setting: "background_theme",
                    value: res.backgroundType
                },
                go_on);
            }],

            setStyle: ["getStyle", "setBackground", function(go_on, res) {
            ...
            togglePurchaseButton: [
                ...
                function(go_on, res)
            {
                if (!!res.hasPremium) { return go_on(); }

                Page._togglePurchasePremiumBackgroundButton({
                    on: res.backgroundType !== "standard"
                });
    ...
    */
    ...
    /** Hide the search form from notes listing
        ...
        async.auto({
            hideSearch: function(go_on) {
                $("#action_search, #action_search i, #search")
                    .removeClass("active btn-primary icon-white");
            ...
            delayForAnimation: ["hideSearch", function(go_on, res) {
                _(go_on).delay(200);
            }],

            scrollToNote: ["delayForAnimation", function(go_on, res) {
                Page._scrollIntoView({
                    container: $("#stacks"),
                    element: Page._selectedNoteListing({})[0]
                });
    ...
    Page.init = function(args, cbk) {
        async.auto({
            addStripeLibrary: function(go_on) {
                if (!!window.Background) { return go_on(); }
            ...
            startPage: ["getLastSelectedNoteId", function(go_on, res) {
                $("#nav, #paper").removeClass("hidden");

                Page.start({select_note_id: res.getLastSelectedNoteId}, go_on);
            }],

            scrollToSelectedNote: ["startPage", function(go_on, res) {
                setTimeout(function() {
                    Page._scrollIntoView({
                        container: $("#stacks"),
                        element: Page._selectedNoteListing({})[0]
    ...
    */
    Page.initShareTokens = function(args, cbk) {
        ...
        if (!!window.Background) { return cbk(); }

        $("#share .tokenfield").tokenfield(tokenOptions)
            .on("tokenfield:createdtoken", function(e) {
                if (Sync.isValidEmail({email: e.attrs.value})) { return; }
    ...
    /** Reset to start

        {
            ...
            [only_update_dom]: <Bool> = false
        }
    */
    Page.logout = function(args, cbk) {
        async.auto({
            clearListings: function(go_on) {
                if (!!args.keep_notes) { return go_on(); }
            ...
            clearStorage: function(go_on) {
                if (!!args.only_update_dom) { return go_on(); }
            ...
            resetAuthentication: ["clearStorage", function(go_on, res) {
            ...
            hardLogout: ["resetAuthentication", function(go_on, res) {
                if (!!args.keep_notes) { return go_on(); }

                Page._logoutRestart({}, go_on);
            }],

            softLogout: ["resetAuthentication", function(go_on, res) {
                if (!args.keep_notes) { return go_on(); }
    ...
    /** Show the sharing modal to confirm sharing of the note
    ...
    */
    ...
    /** Purchase a sync subscription

        {
            email: <String>
            product_description: <String>
            product_icon: <URL String>
            product_key: <String>
            product_name: <String>
            product_price: <Price String>
            purchase_button: <Button DOM Object>
            stripe_key: <String>
        }
    */
        ...
        StripeCheckout
            .configure({
                ...
                token: function(cardToken) {
                    Page._purchasePremiumSubscriptionWithCardToken({
                        card_token: cardToken.id,
                        plan: args.product_key,
                        purchase_button: args.purchase_button
                    });
                }
            })
            .open({
                amount: parseInt(args.product_price),
    ...
    */
    Page.reloadNotesList = function(args, cbk) {
        async.auto({
            ...
            updateListings: ["getNotes", function(go_on, res) {
                async.eachSeries(res.getNotes, function(note, updated) {
                    Page._createOrUpdateListingForNote({
                        id: note.id,
            ...
            selectedId: ["updateListings", function(go_on, res) {
            ...
            sortListings: ["removeListings", function(go_on, res) {
                var stacks = [];

                $("#stacks .stack")
                    .each(function(i, n) { stacks.push($(n)); });
    ...
    */
    ...
    /** Remove a note from the notes view
    ...
    Page.removeSelectedNoteListing = function(args, cbk) {
        ...
        async.auto({
            getNote: function(go_on) {
                ...
                if (!id) { return go_on([0, "Expected node id"]); }

                Data.getNote({id: id}, go_on);
            },

            trashNote: ["getNote", function(go_on, res) {
                if (!res.getNote || !res.getNote.text) { return go_on(); }

                if (!!node.data("trashing")) { return go_on(); }
            ...
            endTrashing: ["trashNote", function(go_on, res) {
    ...
    */
    Page.save = function(args, cbk) {
        ...
        async.auto({
            getNote: function(go_on) {
                if (!id) { return go_on([0, "Expected a selected id"]); }
            ...
            addNote: ["getNote", "noteId", function(go_on, res) {
                if (!!res.getNote) { return go_on(); }

                Data.addNote({id: res.noteId}, go_on);
            }],

            editNote: ["addNote", "noteId", "noteText", function(go_on, res) {
            ...
            updateNoteTime: ["editNote", function(go_on, res) {
                if (!res.editNote) { return go_on(); }

                Page._noteTime({
                    text: res.editNote.text,
                    text_last_modified: res.editNote.text_last_modified
                },
                go_on);
            }],

            pingActivity: ["editNote", function(go_on, res) {
                var now = new Date().toISOString();

                try { localStorage._lastSave = now } catch(e) {}
    ...
    /** Filter the note listings by a query string
    ...
    */
    ...
    /** Makes the note visible
        ...
        This will also remove the note listing confirm trash button
    ...
    */
    Page.selectNote = function(args, cbk) {
        if (!args.id) { return cbk([0, "Expected note id to select"]); }
        ...
        async.auto({
            ...
            adjustNoteTime: ["getNote", function(go_on, res) {
                if (!res.getNote) { return go_on(); }

                Page._noteTime({
                    text: res.getNote.text,
                    text_last_modified: res.getNote.text_last_modified
                },
                go_on);
            }],

            isSelected: ["getNote", function(go_on, res) {
                if (!res.getNote) { return go_on(); }

                go_on(null, Page.getSelectedNoteId({}) === res.getNote.id);
            }],

            previouslySelectedNote: ["isSelected", function(go_on, res) {
            ...
            focusEditArea: ["isSelected", function(go_on, res) {
                if (!res.getNote || !!res.isSelected) { return go_on(); }
            ...
            saveLastSeenNote: ["isSelected", function(go_on, res) {
                if (!res.getNote || !!res.isSelected) { return go_on(); }
            ...
            updatePreviousListing: ["showNote", function(go_on, res) {
                if (!res.previouslySelectedNote) { return go_on(); }

                Page._createOrUpdateListingForNote({
                    id: res.previouslySelectedNote.id
                },
                go_on);
            }],

            updateListing: ["showNote", function(go_on, res) {
                if (!res.getNote || !!res.isSelected) { return go_on(); }
    ...
    /** Select a note listing

        {
            [clear_selection]: <Bool>
            node: <Note Listing DOM Node>
        }
    */
    Page.selectNoteListing = function(args, cbk) {
        if (!args.node) { return cbk([0, "Expected node"]); }
        ...
        async.auto({
            animateListingRemoval: function(go_on) {
                ...
                if (!selected.hasClass("empty")) { return go_on(); }

                selected.addClass("removed");

                _(function() { selected.remove(); }).delay(200);

                go_on();
            },

            clearSelection: function(go_on) {
                if (!args.clear_selection) { return go_on(); }
            ...
            getNote: ["noteId", function(go_on, res) {
                if (!res.noteId) { return go_on([0, "Expected id for note"]); }

                Data.getNote({id: res.noteId}, go_on);
            }],

            unselectListing: ["selectedNode", function(go_on, res) {
            ...
            saveNotes: ["animateSelection", "noteId", function(go_on, res) {
            ...
            showNote: ["getNote", "saveNotes", function(go_on, res) {
    ...
    */
    Page.setBackgroundTheme = function(args, cbk) {
        ...
        if ($("body").hasClass(theme)) { return cbk(); }
        ...
        if (!args.style) { return cbk(); }

        Page.setStyle({style: args.style}, cbk);
    };

    /** Set the focused note text
    ...
    */
    Page.setNoteText = function(args) {
        ...
        if (!args.force && Page.getNoteText({}) === args.text) { return; }
    ...
    */
    ...
    /** Set the style type
        ...
        [style]: <String> = "standard" // "handwriting" | "monospace"
    */
    Page.setStyle = function(args) {
        ...
        textView
            .css({color: reverseColor})
            .animate({color: color}, 150);
    ...
    /** Share a note via a mailto link
    ...
    */
    Page.shareNoteViaCopy = function(args) {
        ...
        if (!emails.split(" ").filter(function(n) { return !!n; }).length) {
            return;
        }
        ...
        var link = "mailto:" +
            encodeURIComponent(emails.split(" ").join(", ")) +
            "?subject=" + encodeURIComponent(title) +
            "&body=" + encodeURIComponent(message) +
            encodeURIComponent(text + sig);
    ...
    /** Share a note via the send note directly button

        {
            button: <Send button DOM node>
        }
    */
    Page.shareNoteViaSendButton = function(args) {
        if (!args.button) { return Client.log({err: [0, "Expected button"]}); }
        ...
        function(err) {
            ...
            if (!!err) { return share.button("error"); }
    ...
    /** Indicate authorization in progress
    ...
    Page.showError = function(args) {
        ...
        if (args.err.status !== undefined) {
            $("#account .error_message")
                .prop("class", "error_message status_" + args.err.status);
        }

        Client.log({err: args.err, path: "page"});
    };

    /** Display a fatal start error

        {
            err: Array<Code, Status>
        }
    */
    Page.showFatalStartError = function(args) {
        if (!args.err) { return Client.log({err: [0, "Expected err"]}); }
        ...
        var toast = "" +
            "<div class=\"span12 toast\">" +
                "<strong>Could not start app.</strong> " +
                reason +
            "</div>";
    ...
    /** Enable the confirm trash button for the selected note
    ...
    */
    Page.showTrashConfirmationForSelectedNote = function(args) {
        ...
        if (listing.is(".empty")) { return Page._selectNextListing({}); }
    ...
    /** Setup the page
        ...
        {
            [restart]: <Bool> = false // Restarting via error
            [select_note_id]: <String>
        }
    */
    Page.start = function(args, cbk) {
        async.auto({
            ...
            showSyncSection: ["setLoggedIn", function(go_on, res) {
                go_on(null, $("#user").removeClass("hidden"));
            }],

            getSelected: ["clearEmpty", function(go_on, res) {
                if (!args.select_note_id) { return go_on(); }

                Data.getNote({id: args.select_note_id}, go_on);
            }],

            getRecent: ["getSelected", "clearEmpty", function(go_on, res) {
                if (!!res.getSelected) { return go_on(); }

                Data.getRecentNotes({limit: 1}, go_on);
            }],

            addNewNote: ["getSelected", "getRecent", function(go_on, res) {
                ...
                if (!!current && !current.destination) { return go_on(); }

                Data.addNote({}, go_on);
            }],

            focusNote: ["addNewNote", function(go_on, res) {
                var n = res.addNewNote || res.getSelected || res.getRecent[0];

                Page.selectNote({id: n.id}, go_on);
            }],

            updateNoteListing: ["focusNote", function(go_on, res) {
                if (!res.addNewNote) { return go_on(); }

                Page._createOrUpdateListingForNote({
                    id: res.addNewNote.id
                },
                go_on);
            }],

            getAnonymousNotice: ["focusNote", function(go_on, res) {
    ...
    */
    Page.toggleRemoteSync = function(args, cbk) {
        async.auto({
            hasRemoteSyncEnabled: function(go_on) {
                Sync.isAccountRemoteSyncEnabled({
            ...
            disableRemoteSync: ["hasRemoteSyncEnabled", function(go_on, res) {
                if (!res.hasRemoteSyncEnabled) { return go_on(); }

                if (!!args.window) { args.window.close(); }
            ...
            getPrivs: ["hasRemoteSyncEnabled", function(go_on, res) {
                if (!!res.hasRemoteSyncEnabled) { return go_on(); }

                Sync.getCachedAccountPrivs({}, go_on);
            }],

            showPurchaseButton: ["getPrivs", "priv", function(go_on, res) {
                if (!!res.hasRemoteSyncEnabled) { return go_on(); }

                var isMissingPriv = !_(res.getPrivs).contains(res.priv);

                if (!isMissingPriv) { return go_on(); }
            ...
            enableRemoteSync: ["showPurchaseButton", function(go_on, res) {
                if (!!res.hasRemoteSyncEnabled || !!res.showPurchaseButton) {
                    return go_on();
                }

                Sync.toggleRemoteSync({
                    on: true,
    ...
    /** Enable or disable the search mode
    ...
    */
    Page.toggleSearchMode = function(args, cbk) {
        async.auto({
            ...
            updateSearchResults: ["toggleSearch", function(go_on, res) {
                if (!!$("#search").is(".active")) { return go_on(); }

                Page.hideSearch({}, go_on);
            }],

            alterSearchFocus: ["toggleSearch", function(go_on, res) {
            ...
            trackToggle: ["toggleSearch", function(go_on, res) {
    ...
    */
    Page.toggleSettings = function(args) {
        async.auto({
            ...
            toggleOptions: ["hasSavedAuth", function(go_on, res) {
                if (!res.hasSavedAuth) { return go_on(); }
    ...
    */
    Page.updateCurrentNote = function(args, cbk) {
        if (!args.id) { return cbk([0, "Expected note"]); }
        ...
        async.auto({
            ...
            trashSelected: ["getNote", function(go_on, res) {
                if (Page.getSelectedNoteId({}) !== args.id) { return go_on(); }

                if (!res.getNote || !res.getNote.deleted) { return go_on(); }

                Page._trashNote({}, go_on);
            }],

            hideListing: ["getNote", function(go_on, res) {
            ...
            updateVisibleNote: ["getNote", function(go_on, res) {
                if (Page.getSelectedNoteId({}) !== args.id) { return go_on(); }

                if (Page._typedRecentlyInNotes({})) { return go_on(); }
                ...
                function(go_on, res)
            {
                ...
                Page._insertListingInSortedList({
                    ...
                    listing: Page._noteListingElementsById({id: args.id}),
                    note: res.getNote
                });
    ...
    Page.updateVisibleElements = function(args, cbk) {
        async.auto({
            deauthorize: function(go_on) {
                if ($("#account").hasClass("authorizing")) { return go_on(); }

                if (!$("#user").hasClass("authorized")) { return go_on(); }

                Sync.hasAuthIncludingCredentials({}, function(err, hasAuth) {
                    if (!!err) { return go_on(err); }

                    if (!!hasAuth) { return go_on(); }
            ...
            getNote: function(go_on) {
                if (!Page.getSelectedNoteId({})) { return go_on(); }

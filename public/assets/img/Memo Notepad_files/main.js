        ...
        async.auto({
            getVersion: function(go_on) {
                $.getJSON("/manifest.json", function(manifest) {
                    ...
                    try { v = manifest.version; } catch(e) {}

                    if (!v) { Client.log({err: [0, "Expected version"]}); }
            ...
            initPage: ["initDb", function(go_on, res) {
                Page.init({}, go_on);
            }],

            initSync: ["initDb", function(go_on, res) {
...
$(function() {
    App._start({}, function(err) {
        if (!!err) { return Page.showFatalStartError({err: err}); }
    });
});

    ...
    PageUnitTests.tests = {
        _createOrUpdateListingForNote: function(cbk) {
            var getListingById = function(id) {
                return Page._noteListingElementsById({id: id})[0];
            };
            ...
            var confirmListingModified = function(listing, go_on) {
                if (!listing) { return go_on([0, "Expected listing"]); }

                var expectedText = Page.titleFromNoteText({text: note.text});
            ...
            async.auto({
                addNote: function(go_on) {
                    Data.addNote({id: note.id, text: note.text}, go_on);
                },

                addListing: ["addNote", function(go_on, res) {
                    Page._createOrUpdateListingForNote({id: note.id}, go_on);
                }],
                ...
                updateNote: ["confirmAddListing", function(go_on, res) {
                    ...
                    Data.editNote({id: note.id, text: note.text}, go_on);
                }],
                ...
                cleanUp: ["confirmUpdateListing", function(go_on, res) {
                    ...
                    Db.removeRow({key: note.id, store: "notes"}, go_on);
                }]
            },
            cbk);
        },
        ...
        _showSearchResults: function(cbk) {
            var notes = [
                {id: "empty", text: ""},
                {id: "hit", text: "search_hit"},
                {id: "miss", text: "search_miss"}
            ];

            async.auto({
                addNotes: function(go_on) {
                    async.eachSeries(notes, function(n, next) {
                        Data.addNote({id: n.id, text: n.text}, function(err) {
                ...
                hideSearchMisses: ["addNotes", function(go_on, res) {
                    Page._showSearchResults({notes: [notes[1]]}, go_on);
                }],

                confirmHidden: ["hideSearchMisses", function(go_on, res) {
                    var empty = Page._noteListingElementsById({id: "empty"});
                    var hit = Page._noteListingElementsById({id: "hit"});
                    var miss = Page._noteListingElementsById({id: "miss"});
                ...
                cleanup: ["confirmHidden", function(go_on, res) {
                    ...
                    async.eachSeries(notes, function(n, removed) {
                        Db.removeRow({key: n.id, store: "notes"}, removed);
                    },
                    go_on);
                }],
        ...
        addRemoteSyncInfoNote: function(cbk) {
            async.auto({
                addDropboxInfo: function(go_on) {
                    Page.addRemoteSyncInfoNote({service: "dropbox"}, go_on);
                },

                addGoogleInfo: ["addDropboxInfo", function(go_on, res) {
                    Page.addRemoteSyncInfoNote({service: "google"}, go_on);
                }],
            ...
            async.auto({
                setNoteText: function(go_on) {
                    ...
                    [
                        "text",
                        "<br>",
                        "<a href=\"http://example.co\">http://example.co</a>",
                        "<br>"
                    ]
                    .forEach(function(n) {
                        $(notes).append(n);
        ...
        searchNotes: function(cbk) {
            var notes = ["a", "b", "c"];
            var visible = function() { return $("#stacks .stack:visible"); };

            async.auto({
                addNotes: function(go_on) {
                    async.eachSeries(notes, function(text, added) {
                        Page.addNewNote({text: text}, added);
                    },
                    go_on);
                },
                ...
                searchNotes: ["addNotes", function(go_on, res) {
                    ...
                    Page.searchNotes({q: notes[1]}, go_on);
                }],
        ...
        selectNoteListing: function(cbk) {
            async.auto({
                ...
                selectA: ["addEmpty", function(go_on, res) {
                    ...
                    Page.selectNoteListing({node: node}, go_on);
                }],
        ...
        setAuthError: function(cbk) {
            ...
            Page.setAuthError({code: "missing_email"});

            if (!assertErrorMessageShowing) { return; }

            Page.setAuthError({code: "missing_password"});

            if (!assertErrorMessageShowing) { return; }

            Page.setAuthError({code: "invalid_email"});
        ...
        setStyle: function(cbk) {
            Page.setStyle({style: "handwriting"});
        ...
        showTrashConfirmationForSelectedNote: function(cbk) {
            async.auto({
                addNote: function(go_on) {
                    Page.addNewNote({text: "string"}, go_on);
                },
        ...
        titleFromNoteText: function(cbk) {
            async.auto({
                leadingSpacesTrimmed: function(go_on) {
                    var title = Page.titleFromNoteText({text: "  \n\nstring"});

...
(function() { // Client
    ...
    */
    Client._getChromeStorageValue = function(args, cbk) {
        if (!args.key) { return cbk([0, "Expected key for chrome storage"]); }

        if (!window.chrome) { return cbk(); }
    ...
    /** Determine if chrome storage exists
    ...
    */
    ...
    Client.analytics = function(args) {
        ...
        if (Client.isChromeApp({}) && !!window.analytics) {
            Client._tracker = analytics
                .getService(Client._ANALYTICS_SERVICE)
                .getTracker(Client._ANALYTICS_TRACKER);
    ...
    */
    Client.clearStorage = function(args, cbk) {
        async.auto({
            getUUID: function(go_on) {
                ...
                if (!!domId) { return go_on(null, domId); }

                Sync.getToken({key: "uuid"}, go_on);
            },

            clearLocalStorage: ["getUUID", function(go_on, res) {
            ...
            clearChromeLocalStorage: ["getUUID", function(go_on, res) {
                if (!window.chrome || !chrome.storage) { return go_on(); }

                chrome.storage.local.clear(go_on);
            }],

            clearChromeSyncStorage: ["getUUID", function(go_on, res) {
                if (!window.chrome || !chrome.storage) { return go_on(); }
    ...
    /** Get a setting
        ...
        {
            setting: <String> = "font_size|style"
        }
    ...
    */
    Client.getSetting = function(args, cbk) {
        if (!args.setting) { return cbk([0, "Expected setting"]); }
    ...
    /** Determine if the client has chrome app details
    ...
    */
    Client.init = function(args, cbk) {
        async.auto({
            ...
            setBackground: ["getBackground", function(go_on, res) {
                Page.setBackgroundTheme({
                    theme: res.getBackground || "standard"
                },
                go_on);
            }],

            setSize: ["getSize", function(go_on, res) {
    ...
    /** Determine if this is a chrome packaged app
    ...
    */
    Client.log = function(args) {
        ...
        if (!!args.path) { console.log(args.path); }

        if (!!args.err) { var e = new Error(); console.log(e.stack); }
    };
    ...
    */
    ...
    /** Get or create the unique id for this particular user

        FIXME: - this is getting reset on logouts
    ...
    */
    Client.registerUUID = function(args, cbk) {
        ...
        if (domUUID) { return cbk(null, domUUID); }
        ...
        async.auto({
            ...
            // Try local storage for xbrowser
                ...
                function(go_on, res)
            {
                ...
                if (!!id && id !== "undefined") { return go_on(null, id); }
    ...
    */
    ...
    /** Set the settings value
        ...
        @returns via cbk
        [<Any>] Stored value or null if did not set
    */
    ...
    /** Track an event
        ...
        {
            path: <String> "category/action/id"
        }
    */
    Client.track = function(args) {
        try {
            ...
            if (!Client._tracker) { return _gaq.push(e); }

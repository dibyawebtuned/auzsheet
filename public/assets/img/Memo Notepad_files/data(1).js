...
(function() { // Data
    ...
    /** Note attributes that are user mutable
    */
    var MUTABLE_NOTE_ATTRIBUTES = [
        "deleted",
        "locked",
        "reminder_at_time",
        "text"
    ...
    /** Sync Object attributes that may note be mutated ever
    */
    var SYNC_OBJECT_IMMUTABLE_ATTRIBUTES = ["created_by", "stream_id"];

    /** Sync Object attributes that mark non user mutable metadata
    */
    var SYNC_OBJECT_MARKER_ATTRIBUTES = [
        "invite_updates_count",
        "response_updates_count",
        "rev"
    ];

    /** Max note length constant

        TODO: - this should be upped to 300k (first gently on the server)
    */
    ...
    /** Add a single note
        ...
        {
            row: <DB Note Row Object>
        }
    */
    var _add = function(args, cbk) {
        if (!args.row.id) { return cbk([0, "Expected id"]); }
    ...
    */
    var _delete = function(args, cbk) {
        if (!args.key) { return cbk([0, "Expected note key to delete"]); }
    ...
    */
    var _get = function(args, cbk) {
        if (!args.key) { return cbk([0, "Expected key for note"]); }
    ...
    */
    var _update = function(args, cbk) {
        if (!args.row || !args.row.id) { return cbk([0, "Expected Row"]); }
    ...
    /** Get empty note ids from rows

        {
            notes: Array<Note Row Object>
        }

        @returns via cbk
        Array<String Note Ids>
    */
    ...
    /** Determine if a note was made dirty after the clean started

        {
            [cleaned_after]: <IS08601 String>
            [current_note]: <Note Object>
        }
    ...
    */
    Data._shouldUpdateDirtyRow = function(args) {
        ...
        if (madeDirtyAfterCleanStart) { return false; }

        return true;
    };

    /** Get a new note row with attributes

        {
            [destination]: <URL String>
            [destination_icon]: <Image URL String>
            [id]: <UUID String> // Required if stream_id is set
            [text]: <String>
        }
    ...
    */
    ...
    /** Get an updated version of a row with local updates applied

        {
            [deleted]: <Int> = undefined
            existingRow: <Note Object>
            [force]: <Bool> = false // Update values even if not changed
            text: <String>
        }

        @returns via cbk
        [<Row Object>] // Empty if no modification was made
    */
    Data._rowWithLocalNoteModifications = function(args, cbk) {
        ...
        MUTABLE_NOTE_ATTRIBUTES
            .filter(function hasModification(type) {
                if (args[type] === undefined) { return false; }
            ...
            .forEach(function applyModification(type) {
                ...
                // Confirm that notes cannot be made longer than note length
    ...
    /** Get a row with updates applied
        ...
        @returns via cbk
        {
            [row]: <Note Row Object> // Absent if there are no modifications
        }
    */
    Data._updatedNoteWithChanges = function(args, cbk) {
        ...
        SYNC_OBJECT_MARKER_ATTRIBUTES
            .filter(function didChangeAttribute(attr) {
    ...
    */
    Data._updateNoteAttributes = function(args, cbk) {
        async.auto({
            ...
            writeChanges: ["getUpdatedRow", function(go_on, res) {
                if (!res.getUpdatedRow) { return go_on(); }

                _update({row: res.getUpdatedRow}, go_on);
            }],

            updateVisibleNote: ["writeChanges", function(go_on, res) {
                if (!res.writeChanges) { return go_on(); }
    ...
    */
    ...
    /** Adjust revs to the current server version
    ...
    Data.adjustNoteRevs = function(args, cbk) {
        if (!Array.isArray(args.revs)) { return cbk([0, "Expected array"]); }

        if (!args.revs.length) { return cbk(); }

        async.each(args.revs, function(note, adjustedRevs) {
            var id = note[0];
            var rev = note[1];
            ...
            async.auto({
                ...
                updateNote: ["getNote", function(go_on, res) {
    ...
    Data.clearEmptyNotes = function(args, cbk) {
        async.auto({
            dbIsReady: function(go_on) {
                if (!Db.isReady) { return go_on([0, "Expected db ready"]); }

                go_on();
            },

            getLastModifiedNotes: ["dbIsReady", function(go_on, res) {
                Db.query({index: "last_modified", store: _STORE}, go_on);
            }],

            getEmptyNoteIds: ["getLastModifiedNotes", function(go_on, res) {
                Data._emptyNoteIdsFromNoteRows({
                    notes: res.getLastModifiedNotes
                },
                go_on);
            }],

            eliminateEmptyNotes: ["getEmptyNoteIds", function(go_on, res) {
                async.each(res.getEmptyNoteIds, function(key, deleted) {
    ...
    */
    Data.deleteNoteById = function(args, cbk) {
        async.auto({
            ...
            eliminateNote: ["hasAuth", function(go_on, res) {
                if (!args.id) { return go_on([0, "Expected id"]); }
    ...
    /** User prompted edit to a local note
    ...
    */
    Data.editNote = function(args, cbk) {
        ...
        if (!args.id) { return cbk([0, "Expected id to edit note"]); }
        ...
        async.auto({
            ...
            noteWithModifications: ["getExistingNote", function(go_on, res) {
                ...
                Data._rowWithLocalNoteModifications({
                    ...
                    existingRow: res.getExistingNote,
                    force: args.force,
                    id: args.id,
                    text: args.text
                },
            ...
            updateNote: ["noteWithModifications", function(go_on, res) {
                if (!res.noteWithModifications) { return go_on(); }

                _update({row: res.noteWithModifications}, go_on);
            }],

            pushChanges: ["updateNote", function(go_on, res) {
                if (!res.noteWithModifications) { return go_on(); }

                setTimeout(function() {
                    Sync.pushChanges({
                        rows: [res.noteWithModifications]
                    },
                    Client.reportErrors({}));
                },
                Math.random() * 1000);
    ...
    */
    Data.getDirtyNotes = function(args, cbk) {
        ...
        function(err, notes) {
            ...
            var dirtyNotes = notes.filter(function(n) {
                // Notes that are just empty placeholders aren't dirty
    ...
    /** Get the first note in an array that is not hidden
    ...
    */
    Data.getFirstVisibleNote = function(args, cbk) {
        if (!Array.isArray(args.notes)) { return cbk([0, "Expected arr"]); }

        async.detectSeries(args.notes, function(note, go_on) {
    ...
    Data.getNextNote = function(args, cbk) {
        Data.getRecentNotes({
            ...
            filter: function(note) {
                if (args.exclude_id === note.id) { return false; }
    ...
    */
    Data.getNote = function(args, cbk) {
        if (!args.id) { return cbk([0, "Expected id for note"]); }
    ...
    /** Get all recent notes

        {
            ...
            [direction]: <String> = prev "prev"|"next"
    ...
    */
    Data.getRecentNotes = function(args, cbk) {
        ...
        Db.query({
            ...
            range: range,
            sort: args.direction || "prev",
            store: _STORE
        },
        cbk);
    };

    /** Determine if a note is hidden
    ...
    */
    ...
    /** Mark notes as clean
        ...
        {
            dirty_before: <ISO 8601 Date String>
            notes: Array<Note DB Row Object>
        }
    */
    Data.markAsClean = function(args, cbk) {
        cbk = cbk || function(){};

        if (!args.dirty_before) { return cbk([0, "Expected dirty date"]); }

        if (!args.notes.length) { return cbk(); }

        var ids = _(args.notes).pluck("id");

        async.each(ids, function(id, go_on) {
            Data.getNote({id: id}, function(err, note) {
                ...
                // Exit early when note made dirty after the clean started
                if (dirtySince > args.dirty_before) { return go_on(); }
    ...
    /** Get all notes that match a query string
    ...
    Data.search = function(args, cbk) {
        ...
        Data.getRecentNotes({
            partial: function(note) {
                if (!args.partial || !matchesQuery(note)) { return; }
    ...
    */
    Data.start = function(args, cbk) {
        async.auto({
            ...
            delayCompletion: ["startDb", function(go_on, res) {
                _(go_on).delay(100);
            }]
        },
        cbk);
    ...
    /** Update notes to the server's version of them
        ...
        {
            note: <Note DB Row Object>
        }
    */
    Data.updateNote = function(args, cbk) {
        if (!args.note) { return cbk([0, "Expected note"]); }
        ...
        async.auto({
            ...
            addNote: ["getExisting", function(go_on, res) {
                if (!!res.getExisting) { return go_on(); }
            ...
            updateNote: ["addNote", function(go_on, res) {
                if (!res.getExisting || !!res.getExisting.dirty_since) {
                    return go_on();
                }

                Data._updateNoteAttributes({
                    existing: res.getExisting,
                    updated: note
                },
                go_on);
            }],
    ...
    */
    Data.updateNoteRev = function(args, cbk) {
        if (!args.id || !args.rev) { return cbk([0, "Expected id and rev"]); }
        ...
        async.auto({
            ...
            updateRow: ["getNote", function(go_on, res) {
                if (!res.getNote) { return go_on(); }
    ...
    */
    ...
    /** Wipe the sync objects and sync paths
    ...
    */
    Data.wipeSyncObjects = function(args, cbk) {
        async.auto({
            ...
            removeSyncObjects: ["getSyncObjects", function(go_on, res) {
                var keys = _(res.getSyncObjects).pluck("id");

                async.forEach(keys, function(k, removed) {
